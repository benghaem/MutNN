...
torch.onnx.export(model, (torch.tensor(rn_loader.get_random(0,batch_len))), "torch_resnet_18.onnx", verbose=True,
...
with torch.no_grad():
    for batch_id in range(0,total_size,batch_len):
        batch = torch.tensor(rn_loader.get_random(batch_id, batch_id + batch_len)).to('cuda')
